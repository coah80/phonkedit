...
public class Renderer2d {
	...
	static void beginScissor(double x, double y, double endX, double endY) {
		...
		int ay = (int) ((client.method_22683().method_4502() - (y + height)) * d);
	 ...
	 * <p>Make sure to link your texture using {@link RenderSystem#setShaderTexture(int, class_2960)} before using this</p>
	 *
	 ...
	 */
	public static void renderTexture(class_4587 matrices, double x0, double y0, double width, double height, float u, float v, double regionWidth, double regionHeight, double textureWidth,
	...
	private static void renderTexturedQuad(Matrix4f matrix, double x0, double x1, double y0, double y1, double z, float u0, float u1, float v0, float v1) {
		...
		buffer.method_22918(matrix, (float) x0, (float) y1, (float) z).method_22913(u0, v1);
		buffer.method_22918(matrix, (float) x1, (float) y1, (float) z).method_22913(u1, v1);
		buffer.method_22918(matrix, (float) x1, (float) y0, (float) z).method_22913(u1, v0);
		buffer.method_22918(matrix, (float) x0, (float) y0, (float) z).method_22913(u0, v0);
	...
	/**
	 * <p>Renders a texture</p>
	 * <p>Does the binding for you, call this instead of {@link #renderTexture(class_4587, double, double, double, double)} or {@link #renderTexture(class_4587, double, double, double, double, float, float, double, double, double, double)} for ease of use</p>
	 *
	 * @param matrices The context MatrixStack
	 ...
	 */
	public static void renderEllipse(class_4587 matrices, Color ellipseColor, double originX, double originY, double radX, double radY, @Range(from = 4, to = 360) int segments) {
		segments = class_3532.method_15340(segments, 4, 360);

		Matrix4f matrix = matrices.method_23760().method_23761();

		float[] colorFloat = getColor(ellipseColor);

		class_287 buffer =class_289.method_1348().method_60827(class_5596.field_27381, class_290.field_1576);
		for (int i = 0; i < 360; i += (int) Math.min(360d / segments, 360 - i)) {
			...
			buffer.method_22918(matrix, (float) (originX + sin), (float) (originY + cos), 0)
					.method_22915(colorFloat[0], colorFloat[1], colorFloat[2], colorFloat[3])
					;
		}
		setupRender();
	 ...
	 */
	public static void renderCircle(class_4587 matrices, Color circleColor, double originX, double originY, double rad, @Range(from = 4, to = 360) int segments) {
	 ...
	 * @param segments     Number of segments that should be rendered. Increasing will decrease speed, decreasing with decrease quality
	 */
	public static void renderEllipseOutline(class_4587 matrices, Color ellipseColor, double originX, double originY, double radX, double radY, @Range(from = 0, to = Long.MAX_VALUE) double width, @Range(from = 0, to = Long.MAX_VALUE) double height, @Range(from = 4, to = 360) int segments) {
		segments = class_3532.method_15340(segments, 4, 360);
		width = class_3532.method_15350(width, 0, radX);
		height = class_3532.method_15350(height, 0, radY);
		...
		for (int i = 0; i <= segments; i++) {
			...
			buffer.method_22918(matrix, (float) (originX + sin), (float) (originY + cos), 0)
					.method_22915(colorFloat[0], colorFloat[1], colorFloat[2], colorFloat[3])
					;
			buffer.method_22918(matrix, (float) (originX + sin1), (float) (originY + cos1), 0)
					.method_22915(colorFloat[0], colorFloat[1], colorFloat[2], colorFloat[3])
					;
		}
		setupRender();
	...
	public static void renderQuad(class_4587 matrices, Color color, double x1, double y1, double x2, double y2) {
		...
		buffer.method_22918(matrix, (float) x1, (float) y2, 0.0F)
				.method_22915(colorFloat[0], colorFloat[1], colorFloat[2], colorFloat[3])
				;
		buffer.method_22918(matrix, (float) x2, (float) y2, 0.0F)
				.method_22915(colorFloat[0], colorFloat[1], colorFloat[2], colorFloat[3])
				;
		buffer.method_22918(matrix, (float) x2, (float) y1, 0.0F)
				.method_22915(colorFloat[0], colorFloat[1], colorFloat[2], colorFloat[3])
				;
	...
	private static void renderRoundedQuadInternal(Matrix4f matrix, float cr, float cg, float cb, float ca, float fromX, float fromY, float toX, float toY, float radC1, float radC2, float radC3,
		...
		for (int i = 0; i < 4; i++) {
			...
			for (float r = i * 90f; r <= (i + 1) * 90f; r += 90 / samples) {
				...
				bufferBuilder.method_22918(matrix, current[0] + sin, current[1] + cos, 0.0F).method_22915(cr, cg, cb, ca);
			}
		}
	...
	private static void renderRoundedOutlineInternal(Matrix4f matrix, float cr, float cg, float cb, float ca, float fromX, float fromY, float toX, float toY, float radC1, float radC2, float radC3,
		...
		for (int i = 0; i < 4; i++) {
			...
			for (float r = i * 90f; r <= (i + 1) * 90f; r += 90 / samples) {
				...
				float cos = cos1 * rad;
				bufferBuilder.method_22918(matrix, current[0] + sin, current[1] + cos, 0.0F).method_22915(cr, cg, cb, ca);
				bufferBuilder.method_22918(matrix, current[0] + sin + sin1 * width, current[1] + cos + cos1 * width, 0.0F)
						.method_22915(cr, cg, cb, ca)
						;
			}
		}
		...
		float rad = current[2];
		bufferBuilder.method_22918(matrix, current[0], current[1] + rad, 0.0F).method_22915(cr, cg, cb, ca);
		bufferBuilder.method_22918(matrix, current[0], current[1] + rad + width, 0.0F).method_22915(cr, cg, cb, ca);
		BufferUtils.draw(bufferBuilder);
	}
	 ...
	 */
	public static void renderRoundedOutline(class_4587 matrices, Color c, double fromX, double fromY, double toX, double toY, float radTL, float radTR, float radBL, float radBR, float outlineWidth,
											float samples) {
		Matrix4f matrix = matrices.method_23760().method_23761();
		float[] color1 = getColor(c);
		float r = color1[0];
	...
	public static void renderLine(class_4587 stack, Color color, double x, double y, double x1, double y1) {
		...
		bufferBuilder.method_22918(m, (float) x, (float) y, 0f)
				.method_22915(colorFloat[0], colorFloat[1], colorFloat[2], colorFloat[3])
				;
		bufferBuilder.method_22918(m, (float) x1, (float) y1, 0f)
				.method_22915(colorFloat[0], colorFloat[1], colorFloat[2], colorFloat[3])
				;
